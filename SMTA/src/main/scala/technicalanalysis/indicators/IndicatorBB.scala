package technicalanalysis.indicators

import org.apache.spark.rdd.RDD
import org.joda.time.DateTime
import technicalanalysis.{MainApp, Record, Utils}
import MainApp.spark
...
object IndicatorBB {
  ...
  def computeBB(stockName: String, inputData: RDD[Record]): Unit = {
    ...
    // WRITE CSVs
    Utils.writeToFile(stockName + "/BB_LOW", MainApp.spark.sparkContext.parallelize(bollinger_bands.zipWithIndex.map(x => (x._2.toDouble + 14D) + ", " + x._1._1).toSeq))
    Utils.writeToFile(stockName + "/BB_MEAN", MainApp.spark.sparkContext.parallelize(bollinger_bands.zipWithIndex.map(x => (x._2.toDouble + 14D) + ", " + x._1._2).toSeq))
    Utils.writeToFile(stockName + "/BB_HIGH", MainApp.spark.sparkContext.parallelize(bollinger_bands.zipWithIndex.map(x => (x._2.toDouble + 14D) + ", " + x._1._4).toSeq))
  }
  ...
  def forecastBB(rdd:RDD[Record], period:Int, forecast:Int): Double = {
    ...
    Utils.roundToDecimal(accuracy_BB.sum / accuracy_BB.length * 100D)
  }
