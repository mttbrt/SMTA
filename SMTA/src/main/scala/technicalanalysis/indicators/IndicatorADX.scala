...
object IndicatorADX {
  ...
  def computeADX(stockName: String, inputData: RDD[Record]): Unit = {
    ...
    // WRITE TO CSV
    Utils.writeToFile(stockName + "/ADX", MainApp.spark.sparkContext.parallelize(this.final_adx.zipWithIndex.map(x => (x._2.toDouble + 29D) + ", " + x._1._2).toSeq))
  }
  ...
  def forecastADX(rdd:RDD[Record], period:Int, forecast:Int): Double = {
    ...
    Utils.roundToDecimal(accuracy.sum/accuracy.length * 100D)
  }
