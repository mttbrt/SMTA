...
object IndicatorMACD {
  ...
  def computeMACD(stockName: String, inputData: RDD[Record]): Unit = {
    ...
    //WRITE CSVs
    Utils.writeToFile(stockName + "/MACD_NORMAL", MainApp.spark.sparkContext.parallelize(macd_list.zipWithIndex.map(x => (x._2.toDouble + 14D) + ", " + x._1._3).toSeq))
    Utils.writeToFile(stockName + "/MACD_SIGNAL", MainApp.spark.sparkContext.parallelize(macd_list.zipWithIndex.map(x => (x._2.toDouble + 14D) + ", " + x._1._4).toSeq))
  }
  ...
  def forecastMACD(rdd:RDD[Record], period:Int, forecast:Int): Double = {
    ...
    Utils.roundToDecimal(accuracy_MACD.sum.toDouble / accuracy_MACD.length.toDouble * 100D)
  }
