...
import technicalanalysis.{DoubleIndicatorValue, MainApp, Record, StraightPartitioner, Utils}
...
object IndicatorRSI {
  ...
  def computeRSI(stockName: String, inputData: RDD[(Long, Record)], period: Int): Unit = {
    Utils.writeToFile(stockName + "/RSI", getRSI(inputData, period).map(x => x._1.toDouble + ", " + x._2.value))
  }
  ...
  def forecastRSI(inputData: RDD[(Long, Record)], period: Int, forecast: Int): Double = {
    ...
    Utils.roundToDecimal((overboughtReliability + oversoldReliability) / (overbought.length.toDouble + oversold.length.toDouble) * 100D)
  }
